...
describe("verify children, find", () => {
    ...
    it("prevAll() To get all previous sibling DOM element within element", () => {
        cy.get('.sales').prevAll().should('have.length', 2)
    })

    it("prevUntil() To get all previous sibling DOM element within element until other element", () => {
        cy.get('#veggie').prevUntil().should('have.length', 6)
    })

    it("siblings() to get all sibling DOM element of element", () => {
